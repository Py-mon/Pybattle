...
from source.creatures.pymon import Pymon
from source.types_ import Armor, Weapon
...
class Humanoid(Pymon):
    ...
    def total_graphics(self) -> str:
        ...
        if self.helmet is not None and self.weapon is not None and self.armor is not None:
            return '\n'.join(map(''.join, zip((self.helmet.graphics + self.armor.graphics).splitlines(), self.weapon.graphics.splitlines())))
