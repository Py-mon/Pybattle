...
class Menu:
    ...
    def frame(self):
        for selection in self.selections:
            if selection == self.selection:
                self._frame.add_frame(CenteredFrame(selection.size, selection.label, colors=[(self.default_color, RectRange(Coord(self.selection.size)))]), selection.location)
            else:
    ...
    def directions(self) -> list[list[Direction]]:
        dirs = [get_directions(self.selection.location, selection.location) for selection in self.selections]
